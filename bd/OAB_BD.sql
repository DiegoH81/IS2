...
BEGIN
    ...
    -- Si la fecha de fin del concepto es antes del fin del año, usar esa fecha
    ...
    -- Determinar desde qué fecha empezamos a contar
    ...
    -- Calcular las ocurrencias según la periodicidad
    CASE p_periodicidad
        WHEN 'Diario' THEN
            dias_periodo := 1;
        WHEN 'Semanal' THEN
        ...
        WHEN 'Quincenal' THEN
            dias_periodo := 15;
        WHEN 'Mensual' THEN
            -- Para mensual, calculamos de manera especial
    ...
    -- Para periodicidades que no son mensuales
...
BEGIN
    ...
    -- Calcular según la periodicidad
    CASE periodicidad
        WHEN 'Diario' THEN
...
BEGIN
    -- Iterar sobre todos los conceptos activos
    FOR rec_concepto IN 
        SELECT 
            idConcepto,
            nombre,
        ...
        WHERE 
            estado = TRUE
            AND fechaInicio <= CURRENT_DATE  -- Ya debería haber iniciado
            AND fechaFin >= CURRENT_DATE     -- Aún está vigente
    LOOP
        contador_conceptos := contador_conceptos + 1;
        
        -- Determinar el periodo en días según la periodicidad
        CASE rec_concepto.periodicidad
            ...
            WHEN 'Quincenal' THEN
                dias_periodo := 15;
            WHEN 'Mensual' THEN
                dias_periodo := 30;  -- Aproximado para mensual
            WHEN 'Personalizado' THEN
        ...
        -- Iniciar desde la fecha de inicio del concepto
		...
		IF rec_concepto.periodicidad = 'Eventual' THEN
		    ...
		    -- Verificar si ya existe una transacción en la fecha de inicio
		    SELECT EXISTS(
		        SELECT 1 
		        FROM transaccion 
		    ...
		    -- No es periódico → saltar al siguiente concepto
		    CONTINUE;
		END IF;

		
        -- Generar transacciones desde la fecha de inicio hasta hoy
        WHILE fecha_transaccion <= CURRENT_DATE LOOP
            -- Verificar si ya existe una transacción para este concepto en esta fecha
            SELECT EXISTS(
                SELECT 1 
                FROM transaccion 
                WHERE 
            ...
            -- Avanzar a la siguiente fecha según el periodo
...
BEGIN
    FOR rec_concepto IN 
        ...
        WHERE 
            ...
            AND idFamilia = p_idFamilia  -- Filtro por familia
    LOOP
        contador_conceptos := contador_conceptos + 1;
...
BEGIN
    ...
    FOR rec IN 
        ...
        WHERE 
            ...
            AND tipo = 'Egreso'
            AND estado = TRUE
            AND fechaFin >= p_fecha_referencia  -- Solo conceptos que aún están vigentes
    LOOP
...
BEGIN
    ...
    -- Consultamos la suma de los Ingresos en el rango de fechas para la familia especificada
    ...
    -- Consultamos la suma de los Egresos en el rango de fechas para la familia especificada
...
BEGIN
    ...
    SELECT 
        ...
        c.nombre,
        c.monto,
        -- NUEVA COLUMNA: Fecha de la próxima facturación
...
BEGIN
    ...
    WHERE t.tipo = 'Egreso'
    AND t.fecha BETWEEN fecha_inicio AND fecha_fin
    AND t.idFamilia = $1;  -- Hacemos referencia explícita al parámetro de entrada

    -- Devolvemos el total de egresos
    RETURN total_egresos;
END;
$_$;
...
BEGIN
    ...
    -- Consultamos la suma de los Egresos en el rango de fechas para el usuario especificado
    SELECT COALESCE(SUM(t.monto), 0) INTO total_egresos
    FROM transaccion t
    WHERE t.tipo = 'Egreso'
    AND t.fecha BETWEEN fecha_inicio AND fecha_fin
    AND t.idUsuario = $1;  -- Hacemos referencia explícita al parámetro de entrada (idUsuario)

    -- Devolvemos el total de egresos para el usuario
    RETURN total_egresos;
END;
$_$;
...
DECLARE
    v_idFamilia INTEGER;
BEGIN
    -- Buscar la id de la familia con el código proporcionado
    SELECT idFamilia
    INTO v_idFamilia
    FROM familia
    WHERE codigoFamilia = p_codigo_familia
    LIMIT 1;  -- Asegúrate de que solo se obtenga un resultado
    
    -- Devolver el id de la familia (si no se encuentra, devuelve NULL)
    RETURN v_idFamilia;
END;
$$;
...
BEGIN
    ...
    -- Devolvemos el total de ingresos
    RETURN total_ingresos;
END;
$_$;
...
BEGIN
    ...
    -- Consultamos la suma de los Ingresos en el rango de fechas para el usuario especificado
    SELECT COALESCE(SUM(t.monto), 0) INTO total_ingresos
    FROM transaccion t
    WHERE t.tipo = 'Ingreso'
    AND t.fecha BETWEEN fecha_inicio AND fecha_fin
    ...
    -- Devolvemos el total de ingresos para el usuario
    RETURN total_ingresos;
END;
$_$;
...
BEGIN
    ...
    FROM transaccion t
    WHERE t.idFamilia = $1  -- Filtramos por la familia
    ORDER BY t.monto DESC;  -- Ordenamos las transacciones por monto (costo), de mayor a menor
END;
$_$;
...
BEGIN
    ...
    SELECT t.idTransaccion, t.fecha, t.monto, t.tipo, t.idConcepto, t.idUsuario
    FROM transaccion t
    WHERE t.idFamilia = $1  -- Aquí usamos $1 para hacer referencia al parámetro de entrada
    AND t.fecha BETWEEN fecha_inicio AND fecha_fin;
END;
$_$;
